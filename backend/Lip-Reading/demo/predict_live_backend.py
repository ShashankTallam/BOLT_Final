...
print(f"Prediction script running. Output: {OUTPUT_FILE}, Stop: {STOP_FILE}, Frame: {FRAME_FILE}", flush=True)

# Ensure clean state
def cleanup_files():
    for f in [OUTPUT_FILE, STOP_FILE, FRAME_FILE]:
        if os.path.exists(f):
            try:
                os.remove(f)
            except Exception:
...
weights_path = os.path.join(os.path.dirname(__file__), '..', 'model', 'model_weights.h5')
...
# Load the predictor
predictor_path = os.path.join(os.path.dirname(__file__), '..', 'model', 'face_weights.dat')
print(f"Loading predictor from: {predictor_path}", flush=True)
...
print("Starting main loop... Check 'stop.txt' to stop.", flush=True)

while True:
    try:
        if os.path.exists(STOP_FILE):
            print(f"Stop signal received from {STOP_FILE}", flush=True)
            break
        ...
        for face in faces:
            ...
            # print(f"Distance: {lip_distance}", flush=True)
            ...
            if lip_distance > 25: # person is talking
                ...
                if not_talking_counter >= NOT_TALKING_THRESHOLD and len(curr_word_frames) + PAST_BUFFER_SIZE == TOTAL_FRAMES: 
                    ...
                    try:
                        with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
                            f.write(f"Predicted: {predicted_word_label}")
                    except Exception as e:
        ...
        # Write frame to file for streaming
        try:
            # Use temporary file to avoid read/write conflicts
            temp_frame = FRAME_FILE.replace(".jpg", "_tmp.jpg")
            cv2.imwrite(temp_frame, frame)
            os.replace(temp_frame, FRAME_FILE)
        except Exception as e:
            print(f"Error writing frame: {e}", flush=True)

        # Small sleep to prevent CPU hogging
        time.sleep(0.01)

    except Exception as outer_e:
        print(f"Error in main loop: {outer_e}")
        time.sleep(0.1)
